...
BEGIN_DATADESC( CTFPasstimeLogic )
	...
	DEFINE_KEYFIELD( m_redGoal, FIELD_STRING, "red_goal"),
	DEFINE_KEYFIELD( m_blueGoal, FIELD_STRING, "blue_goal"),
	...
	DEFINE_INPUTFUNC( FIELD_VOID, "SwapSides", InputSwapSides),
...
void CTFPasstimeLogic::OnEnterGoal( CPasstimeBall *pBall, CFuncPasstimeGoal *pGoal )
{
	...
	//If it wasn't an rocketjump Throw we check for gibigao
	if (!m_bAirborneLaunch)
	{
		if (m_flLaunchTime + streetpass_gibigao_time.GetFloat() <= gpGlobals->curtime)
		{
			return;
		}
	}
...
void CTFPasstimeLogic::LaunchBall( CTFPlayer *pPlayer, const Vector &vecPos, const Vector &vecVel )
{
	...
	//m_bAirborneLaunch = pPlayer->InAirDueToExplosion();
	m_bAirborneLaunch = pPlayer->InAirDueToKnockback();
	GetBall()->m_iLaunchTeam = pPlayer->GetTeamNumber();
...
//-----------------------------------------------------------------------------
void CTFPasstimeLogic::InputSwapSides(inputdata_t& input)
{
	Msg("------- SWAP SIDES STREETPASS -------\n");
	if (m_redGoal != NULL_STRING && m_blueGoal != NULL_STRING)
	{
		Msg("RED GOAL: %s\nBLUE GOAL: %s\n", STRING(m_redGoal), STRING(m_blueGoal));

		CFuncPasstimeGoal* redGoal = dynamic_cast<CFuncPasstimeGoal*>(gEntList.FindEntityByName(NULL, m_redGoal));
		CFuncPasstimeGoal* blueGoal = dynamic_cast<CFuncPasstimeGoal*>(gEntList.FindEntityByName(NULL, m_blueGoal));

		if (!redGoal)
		{
			Warning("%s COULD NOT FIND GOAL UNDER NAME %s\n", GetClassname(), STRING(m_redGoal));
			return;
		}

		if (!blueGoal)
		{
			Warning("%s COULD NOT FIND GOAL UNDER NAME %s\n", GetClassname(), STRING(m_blueGoal));
			return;
		}

		variant_t emptyVariant;
		if (GetBall()->m_iLaunchTeam == TF_TEAM_BLUE)
		{
			redGoal->AcceptInput("Disable", NULL, NULL, emptyVariant, 0);
			blueGoal->AcceptInput("Enable", NULL, NULL, emptyVariant, 0);
		}
		else if(GetBall()->m_iLaunchTeam == TF_TEAM_RED)
		{
			redGoal->AcceptInput("Enable", NULL, NULL, emptyVariant, 0);
			blueGoal->AcceptInput("Disable", NULL, NULL, emptyVariant, 0);
		}

		TFGameRules()->BroadcastSound(255, "Passtime.BallSpawn");
		CrowdReactionSound(GetBall()->m_iLaunchTeam);
	}
}